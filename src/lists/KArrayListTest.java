...
class KArrayListTest {
    ...
    void set_givenIndexOutOfBounds_shouldThrowIndexOutOfBoundsException() {
        ...
        Integer updatedInt = 9;
        assertThrows(IndexOutOfBoundsException.class, () -> {
            list.set(-1, updatedInt);
        });
        assertThrows(IndexOutOfBoundsException.class, () -> {
            list.set(5, updatedInt);
        });
        assertThrows(IndexOutOfBoundsException.class, () -> {
            list.set(100, updatedInt);
        });
    }
    ...
    @Test
    void addAll_whenAddedToBeginning_givenValidInputs_shouldAddAllElementsStartingAtGivenIndex() {
        addFive();
        ArrayList<Integer> itemsToAdd = new ArrayList<>();
        for (int i = 100; i < 107; i++) {
            itemsToAdd.add(i);
    ...
    @Test
    void addAll_whenAddedToMiddle_givenValidInputs_shouldAddAllElementsStartingAtGivenIndex() {
        addFive();
        ArrayList<Integer> itemsToAdd = new ArrayList<>();
    ...
    @Test
    void addAll_whenAddedToEnd_givenValidInputs_shouldAddAllElementsStartingAtGivenIndex() {
        addFive();
        ArrayList<Integer> itemsToAdd = new ArrayList<>();
        ...
        list.addAll(list.size(), itemsToAdd);
        assertEquals(10, list.size());

        Integer[] listShouldBe = {1,2,3,4,5,100,101,102,103,104};
        for (int i = 0; i < 8; i++) {
            assertEquals(listShouldBe[i], list.get(i));
        }
    }
